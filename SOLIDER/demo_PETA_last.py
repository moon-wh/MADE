    ...
    with torch.no_grad():
        for name in os.listdir(args.test_img):
            if not os.path.isdir(name):
                continue
            if name != 'train':
                imags_dir = osp.join(args.test_img+'/'+name)
                for file_name in os.listdir(imags_dir):
                    if file_name == 'query':
                        img_dirs = glob.glob(osp.join(imags_dir+'/'+file_name, '*.jpg'))
                        print(img_dirs)
                        ...
                        pdirs = glob.glob(osp.join(imags_dir +'/'+ file_name, '*'))
                        pdirs.sort()
                        for pdir in pdirs:
                ...
                pdirs = glob.glob(osp.join(args.test_img+'/' + name, '*'))
                pdirs.sort()
                for pdir in pdirs:
                    pid = int(osp.basename(pdir))
                    img_dirs = glob.glob(osp.join(pdir, '*.jpg'))
